...
for ip, name in config['cameras'].items():
    ...
    os.system(f"nice -n 19 {ffmpeg} -y -hide_banner -loglevel error -r 24 -f image2 -start_number {last_hour*120} -i {camdir}/%4d.jpg -frames:v 60 -vf scale=iw/{resolution_divisor}:-1 -c:v libx264 -preset fast -crf 23 {camdir}/hour-{last_hour:02}.mp4")
